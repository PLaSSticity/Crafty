...
// mdb: added this
total = bank_total(pool[0]);
int correct_total = bank_budget - nb_transfers * tx_size;
printf("Bank amount after transactions: %i (should be %i)\n", total, correct_total);
...
static void transaction(int tid, int update_rate_loc, int nb_accounts_loc,
	...
	if ((RAND_R_FNC_aux(seed) % 100) < update_rate_loc) {
		...
		for (i = 0; i < tx_size_loc ; ++i) {
			...
			// Write
			//if ((RAND_R_FNC_aux(seed) % 100) < update_rate_loc) {
			// mdb: added "- 1" so there's a net difference
			TM_SHARED_WRITE(GET_ACCOUNT(loc_pool, sender), sender_new_amount - 1);
...
static void* random_transfer(void *arg)
{
	...
	if ((RAND_R_FNC_aux(seed) % 100) < update_rate_loc) {
		// mdb: added backup of "seed" local variable to support transaction retry
		unsigned int backup_seed = seed;
		TM_BEGIN_EXT(0, 0);
		seed = backup_seed;
