                      ...
                      long resizeRatio, long growthFactor)
{
  ...
  segmentsUnaligned =
    (char *) malloc (sizeof (char) * ALIGNED_HASHTABLE_SIZE * NUM_SEGMENTS +
                     CACHE_LINE_SIZE);
  if (segmentsUnaligned == NULL) {
    return NULL;
  }
  segments =
    (char *) (((uintptr_t) segmentsUnaligned + CACHE_LINE_SIZE - 1) &
              ~(CACHE_LINE_SIZE - 1));
  ...
  for (s = 0; s < NUM_SEGMENTS; s++) {
    if (hashtable_init (GET_SEGMENT (segments, s),
                        initNumBucket, hash, comparePairs, resizeRatio,
                        growthFactor)) {
...
conc_hashtable_containsKey (conc_hashtable_t * concHashtablePtr, void *keyPtr)
{
  ...
  return hashtable_containsKey (GET_SEGMENT (concHashtablePtr->segments, i),
                                keyPtr);
}
...
bool_t
conc_hashtable_insert (conc_hashtable_t * concHashtablePtr, void *keyPtr,
                       void *dataPtr)
{
  long i = concHashtablePtr->hash (keyPtr) % NUM_SEGMENTS;

  return hashtable_insert (GET_SEGMENT (concHashtablePtr->segments, i),
                           keyPtr, dataPtr);
}
...
conc_hashtable_remove (conc_hashtable_t * concHashtablePtr, void *keyPtr)
{
  ...
  return hashtable_remove (GET_SEGMENT (concHashtablePtr->segments, i),
                           keyPtr);
}
...
conc_hashtable_t *
TMconc_hashtable_alloc (long initNumBucket,
                        ulong_t (*hash) (const void *),
                        long (*comparePairs) (const pair_t *, const pair_t *),
                        long resizeRatio, long growthFactor)
{
  ...
  for (s = 0; s < NUM_SEGMENTS; s++) {
    if (TMhashtable_init (GET_SEGMENT (segments, s),
                          initNumBucket, hash, comparePairs, resizeRatio,
                          growthFactor)) {
      free (concHashtablePtr);
      return NULL;
...
void
TMconc_hashtable_free (conc_hashtable_t * concHashtablePtr)
{
  int s;

  for (s = 0; s < NUM_SEGMENTS; s++) {
    TMhashtable_free_buckets (GET_SEGMENT (concHashtablePtr->segments, s));
  }
...
bool_t
TMconc_hashtable_containsKey (TM_ARGDECL conc_hashtable_t * concHashtablePtr,
                              void *keyPtr)
{
  long i = concHashtablePtr->hash (keyPtr) % NUM_SEGMENTS;

  return TMhashtable_containsKey (TM_ARG
                                  GET_SEGMENT (concHashtablePtr->segments, i),
                                  keyPtr);
}


void *
TMconc_hashtable_find (TM_ARGDECL conc_hashtable_t * concHashtablePtr,
                       void *keyPtr)
{
  long i = concHashtablePtr->hash (keyPtr) % NUM_SEGMENTS;

  return TMHASHTABLE_FIND (GET_SEGMENT (concHashtablePtr->segments, i),
                           keyPtr);
}

bool_t
TMconc_hashtable_insert (TM_ARGDECL conc_hashtable_t * concHashtablePtr,
                         void *keyPtr, void *dataPtr)
{
  long i = concHashtablePtr->hash (keyPtr) % NUM_SEGMENTS;

  return TMHASHTABLE_INSERT (GET_SEGMENT (concHashtablePtr->segments, i),
                             keyPtr, dataPtr);
}

bool_t
TMconc_hashtable_remove (TM_ARGDECL conc_hashtable_t * concHashtablePtr,
                         void *keyPtr)
{
  long i = concHashtablePtr->hash (keyPtr) % NUM_SEGMENTS;

  return TMHASHTABLE_REMOVE (GET_SEGMENT (concHashtablePtr->segments, i),
                             keyPtr);
}
