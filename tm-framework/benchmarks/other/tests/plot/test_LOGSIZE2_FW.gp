...
# ARG2 - ACCOUNTS
# ARG3 - THREADS
set output sprintf("|ps2pdf -dEPSCrop - test_%s_FW.pdf", ARG1)
...
set style histogram cluster gap 0 title font ",24" \
    textcolor lt -1 offset character 1.2, 0.8
set style data histograms

set key at graph 0.83,1 vertical Left left reverse samplen 2 width 1 \
  maxrows 2 maxcols 1 font ",30" spacing 4 width 2
#set title sprintf('bank (Nb. accounts=%s, TX\_SIZE=%s, UPDATE\_RATE=%s)', \
#    ARG3, ARG4, ARG5)

set ylabel "Number of Accesses" font ",34" offset 0.8,-0.0
...
plot \
  newhistogram "PSTM" lt -1 fs pattern 2 , \
		PSTM using 4:xtic(1) title "Flush" lw 2 lc 1, \
        ''   u 5         title "Write" lw 2 lc 2, \
	newhistogram "PHTM" lt -1 fs pattern 2, \
        PHTM  u 8:xtic(1) notitle lw 2 lc 1, \
        ''    u     9     notitle lw 2 lc 2, \
	newhistogram "HTPM_{NLP}" lt -1 fs pattern 2, \
        NVHTM_W  u 8:xtic(1) notitle lw 2 lc 1, \
        ''    u     9     notitle lw 2 lc 2, \
  newhistogram "HTPM_{(85\%)}" lt -1 fs pattern 2, \
      NVHTM_B_1 u 11:xtic(1) notitle lw 2 lc 1, \
      ''      u     12                  notitle lw 2 lc 2, \
  newhistogram "HTPM_{(10x)}" lt -1 fs pattern 2, \
      NVHTM_B_8 u 11:xtic(1) notitle lw 2 lc 1, \
      ''      u     12                  notitle lw 2 lc 2, \
  newhistogram "HTPM_{F (85\%)}" lt -1 fs pattern 2, \
      NVHTM_F_1 u 11:xtic(1) notitle lw 2 lc 1, \
      ''      u     12                  notitle lw 2 lc 2, \
  newhistogram "HTPM_{F (10x)}" lt -1 fs pattern 2, \
      NVHTM_F_8 u 11:xtic(1) notitle lw 2 lc 1, \
      ''      u     12                  notitle lw 2 lc 2, \
#
