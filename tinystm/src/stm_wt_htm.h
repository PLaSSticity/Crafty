...
#ifndef _STM_WT_HTM_H_
#define _STM_WT_HTM_H_

// To use in combination with HTM
#define WT_HTM_ATOMIC_CAS_FULL(a, e, v) ({ \
  int res; \
  if (HTM_IN_TRANSACTION()) { res = 1; *(a) = *(v); } \
  else { res = ATOMIC_CAS_FULL(a, e, v)); } \
  res; \
})
#define WT_HTM_ROLLBACK(tx, cause) ({ \
  HTM_COMMIT(); \
  stm_rollback(tx, cause); \
})
#define WT_HTM_ACCESS() ({ \
  HTM_currTxSize++; \
  if (HTM_currTxSize > HTM_maxTxSize) { \
    HTM_COMMIT(); \
    HTM_BEGIN(); \
  } \
})
...
static INLINE int
stm_wt_htm_validate(stm_tx_t *tx)
{
  r_entry_t *r;
  int i;
  stm_word_t l;

  PRINT_DEBUG("==> stm_wt_htm_validate(%p[%lu-%lu])\n", tx, (unsigned long)tx->start, (unsigned long)tx->end);
 ...
 */
static INLINE int
stm_wt_htm_extend(stm_tx_t *tx)
{
  stm_word_t now;

  PRINT_DEBUG("==> stm_wt_htm_extend(%p[%lu-%lu])\n", tx, (unsigned long)tx->start, (unsigned long)tx->end);
...
static INLINE void
stm_wt_htm_rollback(stm_tx_t *tx)
{
  int i;
  w_entry_t *w;
  stm_word_t t;

  PRINT_DEBUG("==> stm_wt_htm_rollback(%p[%lu-%lu])\n", tx, (unsigned long)tx->start, (unsigned long)tx->end);
...
static INLINE void
stm_wt_htm_add_to_rs(stm_tx_t *tx, stm_word_t version, volatile stm_word_t *lock)
{
  r_entry_t *r;
...
/* TODO: HTM
 *
 * change this to have the chunking: need a counter
 * in the HTM side to know where to place the
 * _xend/_xcommit barrier
 *
 * Change the GET_LOCK and ATOMIC_LOAD_ACQ
 *   no CAS inside of the HTM transaction
 *
 * */

static INLINE stm_word_t
stm_wt_htm_read(stm_tx_t *tx, volatile stm_word_t *addr)
{
  volatile stm_word_t *lock;
  stm_word_t l, l2, value, version;
  w_entry_t *w;

  PRINT_DEBUG2("==> stm_wt_htm_read(t=%p[%lu-%lu],a=%p)\n", tx, (unsigned long)tx->start, (unsigned long)tx->end, addr);
 ...
 restart_no_load:
  if (likely(!LOCK_GET_WRITE(l))) {
    ...
    stm_wt_htm_add_to_rs(tx, version, lock);

    /* Valid version? */
    if (unlikely(version > tx->end)) {
      /* No: try to extend first (except for read-only transactions: no read set) */
      if (tx->attr.read_only || !stm_wt_htm_extend(tx)) {
...
stm_wt_htm_write(stm_tx_t *tx, volatile stm_word_t *addr, stm_word_t value, stm_word_t mask)
{
  volatile stm_word_t *lock;
  ...
  PRINT_DEBUG2("==> stm_wt_htm_write(t=%p[%lu-%lu],a=%p,d=%p-%lu,m=0x%lx)\n",
...
# endif /* CONFLICT_TRACKING */
 ...
 acquire_no_check:
  ...
  if (tx->w_set.nb_entries == tx->w_set.size)
    stm_rollback(tx, STM_ABORT_EXTEND_WS);
  w = &tx->w_set.entries[tx->w_set.nb_entries];
  // TODO: when in HTM, avoid the CAS (changed here)
  if (WT_HTM_ATOMIC_CAS_FULL(lock, l, LOCK_SET_ADDR_WRITE((stm_word_t)w)) == 0)
    goto restart;
...
static INLINE stm_word_t
stm_wt_htm_RaR(stm_tx_t *tx, volatile stm_word_t *addr)
{
  /* TODO same as fast read but no need to add into the RS */
  return stm_wt_htm_read(tx, addr);
}

static INLINE stm_word_t
stm_wt_htm_RaW(stm_tx_t *tx, volatile stm_word_t *addr)
{
#ifndef NDEBUG
  stm_word_t l;
  w_entry_t *w;
...
static INLINE stm_word_t
stm_wt_htm_RfW(stm_tx_t *tx, volatile stm_word_t *addr)
{
  /* Acquire lock as write. */
  stm_wt_htm_write(tx, addr, 0, 0);
  /* Now the lock is owned, read directly from memory is safe. */
  return *addr;
}

static INLINE void
stm_wt_htm_WaR(stm_tx_t *tx, volatile stm_word_t *addr, stm_word_t value, stm_word_t mask)
{
  /* Probably no optimization can be done here. */
  stm_wt_htm_write(tx, addr, value, mask);
}

static INLINE void
stm_wt_htm_WaW(stm_tx_t *tx, volatile stm_word_t *addr, stm_word_t value, stm_word_t mask)
{
#ifndef NDEBUG
  stm_word_t l;
  w_entry_t *w;
...
static INLINE int
stm_wt_htm_commit(stm_tx_t *tx)
{
  w_entry_t *w;
  stm_word_t t;
  int i;

  PRINT_DEBUG("==> stm_wt_htm_commit(%p[%lu-%lu])\n", tx, (unsigned long)tx->start, (unsigned long)tx->end);
...
# endif /* ! IRREVOCABLE_IMPROVED */
  ...
  if (unlikely(tx->start != t - 1 && !stm_wt_htm_validate(tx))) {
    /* Cannot commit */
