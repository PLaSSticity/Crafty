...
public:
  ...
  BPlusTree() : depth(0), root(new_leaf_node()) {
    //DEBUG
...
private:
  ...
  enum NodeType {
    ...
    static unsigned slow_leaf_position_for(TM_ARGDECL const KEY & key, const KEY * keys, unsigned long num_keys){
      //Simple linear search.Faster for small values of N or M
      unsigned k = 0;
    ...
    __attribute__((transaction_safe)) void slow_inner_insert_nonfull(TM_ARGDECL InnerNode * node, unsigned long current_depth, KEY & key, VALUE & value){
      ...
      if (current_depth - 1 == 0) {
        ...
        if (temp_node == NULL) {
          temp_node = new_leaf_node();
          //TODO:MEMORY LEAK ! !!
          SLOW_PATH_SHARED_WRITE_P(node->children[index], temp_node);
        }
