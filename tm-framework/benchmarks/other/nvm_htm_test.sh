	...
	do
		...
		do
			...
			do
				...
				for t in 1 2 4 8 10 12 14 20 28 46 54 # 1 2 3 4 5 6 # 
				do
...
do
    ...
    do
        for l in $LOG_SIZE2 $LOG_SIZE3 $LOG_SIZE4 # $LOG_SIZE4 #
        do
            for a in 4 # 1 2 3 4 5 # sort-same-thr / array / no-sort / diff-thrs / batch writes
            do
                # TODO: move for a=6
                # a=5
                S$s DO_CHECKPOINT=$d LOG_SIZE=$l \
                    SORT_ALG=$a THRESHOLD=0.0" \
                    $BUILD_SCRIPT htm-sgl-nvm SOL"$d"_l"$l"_p"$p" >/dev/null
                # a=6
                bench_1 SOL"$d"_l"$l"_p"$p"_a"$a"_th
                bench_updates SOL"$d"_l"$l"_p"$p"_a"$a"_up
            done
        done
    done
done
...
for l in $LOG_SIZE2 $LOG_SIZE3 $LOG_SIZE4 # $LOG_SIZE4 #
do
    for p in $FILTER1 $FILTER2 $FILTER3
    do
        # TODO: move for a=6
        # a=5
        MAKEFILE_ARGS="SOLUTION=4 DO_CHECKPOINT=5 LOG_SIZE=$l \
            ...
            $BUILD_SCRIPT htm-sgl-nvm SOL"$d"_l"$l"_p"$p" >/dev/null
        # a=6
        bench_1 SOL5_l"$l"_p"$p"_a5_th
        bench_updates SOL5_l"$l"_p"$p"_a5_up
    done
done
